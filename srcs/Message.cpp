...
=======
/*   Updated: 2022/03/18 17:03:35 by thallard         ###   ########lyon.fr   */
>>>>>>> 38337080d13ebbd16f2bd840f1d52eeeeff12318
/*                                                                            */
/* ************************************************************************** */
...
void Message::setup()
{
    ...
    _position = _true_position;
    _D = Matrix(3, 1);

    _D(0, 0) = cos(_direction(0, 0)) * cos(_direction(1, 0));
    _D(1, 0) = sin(_direction(0, 0)) * cos(_direction(1, 0));
    _D(2, 0) = sin(_direction(1, 0));
}

void Message::update(double dt){
    // _speed = (_speed * getRotationMatrix() * _direction * dt); //+ (_acceleration * dt * dt * 0.5);
    _position = _position + _speed * _D * dt + 0.5 * _acceleration * dt * dt;
}
